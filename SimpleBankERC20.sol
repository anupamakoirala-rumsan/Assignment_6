...
import "https://github.com/OpenZeppelin/openzeppelin-solidity/blob/v3.4.0/contracts/token/ERC20/ERC20.sol";
...
contract SimpleBank is ERC20 {
   ...
   address public owner;
    ...
    constructor(address _reserveToken,string memory _name,string memory _symbol)public ERC20(_name,_symbol)  {
    ...
    function deposit(uint256 numtokens) external  isEnrolled returns (bool)  {
    ...
    uint256 priceForTokens = numtokens.mul(2);

        require(
            // Here we are casting _erc20Token to the IERC20 interface, 
            // then calling transferFrom on the casted contract
    ...
    function withdraw(uint withdrawAmount) external  isEnrolled returns (bool) {
        ...
        require(balanceOf(msg.sender)>= withdrawAmount,"insufficent amount");
        uint256 priceforTokens =withdrawAmount.div(2);
        require(IERC20(_erc20token).transfer(msg.sender,priceforTokens),"Transfer failed");
